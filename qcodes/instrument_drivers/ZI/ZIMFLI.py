# -*- coding: utf-8 -*-
"""
File: ZIMFLI.py - QCoDeS driver for the ZurichInstrument LockIn Amplifier MFLI
...
from qcodes.instrument.sync import Sync, ExplicitSync, SyncCommand, SyncInstrument, SyncParameter # RepeatedSync

logNorm = logging.getLogger(__name__)         # normal logging strings
logSync = logging.getLogger(__name__+".sync") # logger for device messages in sync commands
...
class PIDChannel(InstrumentChannel):
    ...
    ATTENTION: this function is not tested because there was no device available with this
    function installed.
    Parameters:
...
class Sweep(MultiParameter):
    ...
    def _prepareSweeper(self):
        """
        Helper function to prepare the sweep.
        """
        ...
        if signals == []:
            raise RuntimeError('No signals selected! Can not perform sweep.')

        if self._instrument.sweep_correctly_built is False:
            raise RuntimeError('The sweep has not been correctly built.' +
        ...
        self.streamsettings = []  # This list keeps track of the pre-sweep settings
        for sigstr in signals:
            ...
            try:
                ...
                setting = 0
            self.streamsettings.append(setting)
        ...
        self.sweeperStart = None
        self.sweeperTimeout = self._instrument.submodules['sweeper_channel'].sweeper_timeout()


    def _startSweeper(self):
        """
        Helper function to start the sweep.
        """
        if hasattr(self._instrument, 'sweep_correctly_built'):
            if self._instrument.sweep_correctly_built is False:
                raise RuntimeError("Sweep not built")
        else:
            raise RuntimeError("Sweep not defined")
        self._instrument.sweeper.execute()
        self.sweeperStart = time.time()


    def _checkSweeper(self):
        """
        Helper function to check the sweep. If one sweep is completed, check if
        the next one has to be started and do this.
        Returns True if all sweeps are finished, otherwise False
        """
        sweeper = self._instrument.sweeper
        if sweeper.finished():
            if self.sweeperStart == None:
                return True
            logSync.info("Sweeper execution time =", time.time() - self.sweeperStart, "sec")
            self.sweeperStart = None
            return True
        if (time.time() - self.sweeperStart) > self.sweeperTimeout:
            # If for some reason the sweep is blocking, force the end of the
            # measurement.
            logSync.error("Sweep still not finished, forcing finish...")
    ...
    def get(self):
        ...
        The code is divided into pieces for the SyncInstrument interface

        Returns:
            tuple: Tuple containing N numpy arrays where N is the number
              of signals added to the sweep.
        Raises:
            RuntimeError: If no signals have been added to the sweep
            RuntimeError: If a sweep setting has been modified since
                the last sweep, but Sweep.build_sweep has not been run
        """
        self._prepareSweeper()
        self._startSweeper()
        while not self._checkSweeper():  # Wait until the sweep is done/timeout
            time.sleep(0.2)  # Check every 200 ms whether the sweep is done
        return self._getDataFromSweeper()

    
    def _getDataFromSweeper(self, single_signal = None):
        """
        Helper function to get the data from the sweep.
        """
        daq = self._instrument.daq
        signals = self._instrument._sweeper_signals
        if single_signal is not None:
            for s in signals:
                if s.endswith(single_signal):
                    signals = [s]
                    break
        sweeper = self._instrument.sweeper
        ...
        # if single_signal is None: - unclear
        sweeper.unsubscribe('*')
        for (state, sigstr) in zip(self.streamsettings, signals):
            path = '/'.join(sigstr.split('/')[:-1])
...
#    def _parsesweepdata(self, sweepresult):
        ...
        for signal in signals: # self._instrument._sweeper_signals:
            path = '/'.join(signal.split('/')[:-1])
            attr = signal.split('/')[-1]
            d = data[path][0][0][trans[attr]]
            returndata.append(d)
...
class Scope(MultiParameter):
    ...
    def prepare_scope(self):
        ...
        logNorm.info('Preparing the scope')
    ...
    def get_raw(self):
        ...
        logNorm.info('Scope get method called')
        ...
        while (zi_error or timedout) and error_counter < num_retries:
            ...
            try:
                ...
                logNorm.debug('Starting ZI scope acquisition.')
                ...
                while progress < 1:
                    logNorm.debug('Scope progress is {}'.format(progress))
                    progress = scope.progress()
                ...
                if timedout or zi_error:
                    logNorm.warning('[-] ZI scope acquisition attempt {} '
                    ...
                    logNorm.info('[+] ZI scope acquisition completed OK')
                    rawdata = scope.read()
                    if 'error' in rawdata:
                ...
                if error_counter >= num_retries:
                    logNorm.error('[+] ZI scope acquisition failed, maximum number'
        ...
        logNorm.info('scope get method returning after {} s'.format(t_stop -
                                                                t_start))
        return data
...
class SyncLockInCommand(SyncCommand):
    """
    Stores informations for one command
    """
    def __init__(self, parameter: str, values: list, start: list, dur: float ):
        """
        parameter: the string for the local parameter to sweep
        values: list of values to be set
        start:
        dur: measurement duration at each point
        """
    ...
    def debug(self):
        ...
        print("DBG: Command: Param=", self._param, "Values=", self._values,
              "Start=", self._start, "Duration=", self._duration)


class SyncOutputParameter(SyncParameter):
    """
    Parameter class for the Sync Sweep
    """
    def __init__(self, name: str,
                 instrument: 'Instrument',
                 parameter: str):
    ...
    def sync_set(self, values, sync: Sync = None) -> Sync:
        ...
        """
        if sync is None:
            # Creation of new Sync object is not possible because we didn't know the timing
            raise NotImplementedError()
        expsync = sync.as_explicit()
        for es in expsync.begin:
            print("DBG: ***sync*** ", es, sync._duration )
        sync.add_command(self._instr,
                         SyncLockInCommand(self._param, values,
                                            expsync.begin,
                                            sync._duration) )
        return sync

    def sync_get(self, sync: Sync) -> Callable[[], np.ndarray]:
    ...
    def is_running(self) -> bool:
        ...
        """
        return self._instr.Sweep._checkSweeper()


class SyncInputParameter(SyncParameter):
    """
    Parameter class for the Sync Sweep
    """
    def __init__(self, name: str,
    ...
    def sync_get(self, sync: Sync) -> Callable[[], np.ndarray]:
        ...
        if sync is None:
            raise NotImplementedError()
        self._instr.Sweep.sweeperStart = None
        self._instr.add_signal_to_sweeper(1, self._param)

        # Returns the object to get the data
        return self._getData

    def _getData(self):
        return self._instr.Sweep._getDataFromSweeper(self._param)

    def is_running(self) -> bool:
        """
        Helper function to check if the complete sequence is finished (return True)
        or if ist is running (return False). This routine might block if one
        measurement is finished and the next measurement time is in the future.
...
class ZIMFLI(Instrument, SyncInstrument):
    """
    ...
    def __init__(self, name: str, device_ID: str, **kwargs) -> None:
        ...
        if softSweep or "MD" in self.options:
            ...
            #########################
            # All parameter for the sync model
            self.add_parameter('sync_freq1',
                               parameter_class=SyncOutputParameter,
                               parameter='Osc 1 Frequency')
            self.add_parameter('sync_auxout1',
                               parameter_class=SyncOutputParameter,
                               parameter='Aux Out 1 Offset')
            self.add_parameter('sync_auxout2',
                               parameter_class=SyncOutputParameter,
                               parameter='Aux Out 2 Offset')
            self.add_parameter('sync_auxout3',
                               parameter_class=SyncOutputParameter,
                               parameter='Aux Out 3 Offset')
            self.add_parameter('sync_auxout4',
                               parameter_class=SyncOutputParameter,
                               parameter='Aux Out 4 Offset')
            self.add_parameter('sync_phase1',
                               parameter_class=SyncOutputParameter,
                               parameter='Demod 1 Phase Shift')
            self.add_parameter('sync_phase2',
                               parameter_class=SyncOutputParameter,
                               parameter='Demod 2 Phase Shift')
            self.add_parameter('sync_out1ampl2',
                               parameter_class=SyncOutputParameter,
                               parameter='Output 1 Amplitude 2')
            self.add_parameter('sync_out1off',
                               parameter_class=SyncOutputParameter,
                               parameter='Output 1 Offset')
            if 'MD' in self.options:
                self.add_parameter('sync_phase3',
                                   parameter_class=SyncOutputParameter,
                                   parameter='Demod 3 Phase Shift')
                self.add_parameter('sync_phase4',
                                   parameter_class=SyncOutputParameter,
                                   parameter='Demod 4 Phase Shift')
                self.add_parameter('sync_freq2',
                                   parameter_class=SyncOutputParameter,
                                   parameter='Osc 2 Frequency')
                self.add_parameter('sync_out1ampl4',
                                   parameter_class=SyncOutputParameter,
                                   parameter='Output 1 Amplitude 4')
                self.add_parameter('sync_out2ampl8',
                                   parameter_class=SyncOutputParameter,
                                   parameter='Output 2 Amplitude 8')
                self.add_parameter('sync_out2off',
                                   parameter_class=SyncOutputParameter,
                                   parameter='Output 2 Offset')
            
            self.add_parameter('sync_x',
                                   parameter_class=SyncInputParameter,
                                   parameter='X')
            self.add_parameter('sync_y',
                                   parameter_class=SyncInputParameter,
                                   parameter='Y')
            self.add_parameter('sync_r',
                                   parameter_class=SyncInputParameter,
                                   parameter='R')
            self.add_parameter('sync_phase',
                                   parameter_class=SyncInputParameter,
                                   parameter='phase')
            self.add_parameter('sync_x_rms',
                                   parameter_class=SyncInputParameter,
                                   parameter='Xrms')
            self.add_parameter('sync_y_rms',
                                   parameter_class=SyncInputParameter,
                                   parameter='Yrms')
            self.add_parameter('sync_r_rms',
                                   parameter_class=SyncInputParameter,
                                   parameter='Rrms')
            self.add_parameter('sync_phase_pwr',
                                   parameter_class=SyncInputParameter,
                                   parameter='phasePwr')
            self.add_parameter('sync_freq',
                                   parameter_class=SyncInputParameter,
                                   parameter='Freq')
            self.add_parameter('sync_freq_pwr',
                                   parameter_class=SyncInputParameter,
                                   parameter='FreqPwr')
            self.add_parameter('sync_in1',
                                   parameter_class=SyncInputParameter,
                                   parameter='In1')
            self.add_parameter('sync_in2',
                                   parameter_class=SyncInputParameter,
                                   parameter='In2')
            self.add_parameter('sync_in1_pwr',
                                   parameter_class=SyncInputParameter,
                                   parameter='In1Pwr')
            self.add_parameter('sync_in2_pwr',
                                   parameter_class=SyncInputParameter,
                                   parameter='In2Pwr')
        # end of 'if softSweep or "MD" in self.options:'
    ...
    def _getter(self, module: str, number: int,
        ...
        logNorm.debug("getting %s", querystr)
        if mode == 0:
    ...
    def remove_signal_from_sweeper(self, demodulator, attribute):
        ...
        signalstring = ('/' + self.device +
                        '/demods/{}/sample/{}'.format(demodulator-1,
                                                      attribute))
        if signalstring not in self._sweeper_signals:
            logNorm.warning('Can not remove signal with {} of'.format(attribute) +
    ...
    def bufferedReader( self, demod_index, total_time, dolog=False, copyFreq=False,
        ...
        """
        if not 'demod{}'.format(demod_index) in self.submodules:
        ...
        while True:
            ...
            if dolog:
                print( 'bufferedReader: time={0:6.2f}s,  data length={1}'.format( time_elapsed, len(data[path]['x'])) )
    ...
    """
    Preparation interface for SyncInstrument
    """
    def prepare(self, sync: Sync, sync_command: SyncCommand):
        ...
        Args:
            sync: the current Sync object
            sync_command: the current command for this device
        """
        print("DBG: prepare", sync_command)
        # save the command for later usage (timing)
        self.syncCmd = sync_command
        #        self.syncCmd.debug() # print out all informations - remove it later

        # setting fixed parameters for the sweeper channel
        sc = self.submodules['sweeper_channel']
        sc.param( self.syncCmd._param )
        sc.endless('OFF')
        sc.start( self.syncCmd._values[0] )
        sc.stop( self.syncCmd._values[-1] )
        sc.samplecount( len(self.syncCmd._values) )
        sc.loopcount( 1 )
        sc.mode( 'sequential' )
        sc.xmapping( 'linear' ) # TODO: ob Lin/Log sollte automatisch erkannt werden
        sc.bandwidth_mode( 'fixed' )
        sc.averaging_time( self.syncCmd._duration / len(self.syncCmd._values) ) # time for one sample point
        sc.sweeper_timeout( self.syncCmd._duration * 1.1 ) # duration for all sample points as a timeout
...
#        set_tim = sc.settling_time()       # minimum time [sec] from setting and measure
        ...
        #if set_tim * avg_sam > avg_tim:
        #    print("***ERROR: settling", set_tim*avg_sam, "greater than averaging time", avg_tim)
        self.Sweep.build_sweep() # to calculate the sweeptime correctly
        self.Sweep._prepareSweeper() # to send the informations to the device


    def run(self):
        """Helper function to start the Sync-Sweep manually"""
        if self.Sweep.sweeperStart == None:
            # Now start the Sweeper
            self.syncIndex = 0
            self.Sweep._startSweeper()
        else:
            raise RuntimeError("Sweeper is running")
